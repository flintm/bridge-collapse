...
if(length(grp)==1){
  ...
  if (SAVE){
#     grp[[1]]
#     pdf(file = file.path("Plots",paste(SAVENAME,"pdf",sep=".")), width = SIZE[1], height = SIZE[2],useDingbats = F)
#     print(grp[[1]])
#     dev.off()
    ggsave(grp[[1]],filename = file.path("Plots",paste(SAVENAME,"pdf",sep=".")), width = SIZE[1], height = SIZE[2])
  }
}
if(length(grp)==2){
...
if(length(grp)==10){
  ...
  if(embedFonts){
    embedFonts(file = file.path("Plots",paste(SAVENAME,"pdf",sep=".")))
  }
}
