...
# 2019-02-22 Google Maps now requires an API and this option is disabled throughout. This
# affects options Figures 1 and 2 but renders PlotIndividualBridges useless. An issue
# has been entered fix PlotIndividualBridges if users have set up their own API key.
# Otherwise the script works as expected with R 3.5.2 and ggplot2 3.1.0, thanks to 
# updates to almost all plotting scripts to accomodate package changes.
...
rm(dirM,folders,dirsCreate,dirsGit)
...
#        type         = one of {"1991", "ageContinuous", "ageDiscrete"}
...
source(file.path("Scripts","Plotting","PlotAllBridgesMap.R"))
load(file.path("Data","df.USbridges.water.RData"))
...
AllBridgesMap             <- PlotAllBridgesMap(df.USbridges.water, ggmap = FALSE, type = "ageDiscrete",
                                                outputFormat = "png",SAVE=TRUE, gs_path = gs_path)
...
pdf(file=file.path("Plots","Fig2.pdf"), width = controls$SIZE[1], height = controls$SIZE[2], title = "Fig 2 Map of Collapsed Bridges")
grid.newpage()
...
if(controls$embedFonts){
  ...
  embed_fonts(file.path("Plots","Fig2.pdf"))
}
...
source(file.path("Scripts","Plotting","PlotMatTypesBarChart.R"))
load(file.path("Data","df.USbridges.water.RData"))
load(file.path("Data","df.Fail.NBI.Gage.RData"))
df.USbridges.water <- df.USbridges.water[,c("ITEM8","STFIPS","ITEM27","ITEM43A","ITEM43B","ITEM106","LATDD","LONGDD")]
BarP               <- PlotMatTypesBarChart(df.Fail.NBI.Gage[rowsToView,], df.USbridges.water, SAVE = TRUE, embedFonts = embedFonts, gs_path = gs_path)
...
Q_units     <- "m3s"
source(file.path("Scripts","Plotting","PlotCorrsMakeGrid.R"))
...
source(file.path("Scripts","Plotting","PlotGageTrendsMap.R"))
load(file.path("Data","df.Fail.NBI.Gage.RData"))
...
load(file.path("Data","df.Fail.NBI.Gage.RData"))
rowsToPlot   <- rowsToView[1]
BridgeOnly   <- FALSE
saveFormat   <- "pdf"
source(file.path("Scripts","Plotting","PlotIndividualBridges.R"))
