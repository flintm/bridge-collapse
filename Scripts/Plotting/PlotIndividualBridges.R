...
# Deprecated as 2019--not set up for Google API key, which ggmap requires even if OSM or another map source is selected.
...
if(!("createScale" %in% ls(.GlobalEnv))) source(file.path("Scripts","Plotting","createScale.R"))
if(!("gcd_slc" %in% ls(.GlobalEnv))) source(file.path("Scripts","Helper","gcd_slc.R"))
...
if(!("df.Fail.NBI.Gage" %in% ls(.GlobalEnv))) source(file.path("Data","df.Fail.NBI.Gage.R"))
if(!("df.States" %in% ls(.GlobalEnv))) load(file.path("Data","StateCountyCityData.RData"))
