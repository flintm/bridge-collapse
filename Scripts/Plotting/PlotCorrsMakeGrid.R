...
if(!("df.Fail.NBI.Gage" %in% ls(.GlobalEnv))) load(file.path("Data","df.Fail.NBI.Gage.RData"))
...
if(!("PlotAllCorrs" %in% ls(.GlobalEnv))) source(file.path("Scripts","Plotting","PlotAllCorrs.R"))
...
ls.corrs.fail <- CorrelationsFailMaxEvents(SAVE = FALSE)
...
if (OUTPUT_TYPE == "DAILY-USGS-2x2-INDIV"){
  ...
  ggsave(filename = file.path("Plots","Fig5a.pdf"), plot = p,width=3.5,height=3.4)
  p <-CorrPlots[["USGS.T.shape"]][[bases[2]]] + theme(plot.margin   = unit(pmarg, "in"))
  ggsave(filename = file.path("Plots","Fig5b.pdf"), plot = p,width=3.5,height=3.4)
  p <-CorrPlots[["USGS.partd.shape"]][[bases[3]]] + theme(plot.margin   = unit(pmarg, "in"))
  ggsave(filename = file.path("Plots","Fig5c.pdf"), plot = p,width=3.5,height=3.4)
  p <-CorrPlots[["USGS.partip.shape"]][[bases[4]]] + theme(plot.margin   = unit(pmarg, "in"))
  ggsave(filename = file.path("Plots","Fig5d.pdf"), plot = p,width=3.5,height=3.4)
  if (embedFonts){
    require(extrafont)
    Sys.setenv(R_GSCMD=gs_path)
    for (file in paste0("Fig5",c("a","b","c","d"),".pdf")){
      embed_fonts(file.path("Plots",file))
    }
  }
}
